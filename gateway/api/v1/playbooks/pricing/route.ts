...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!tier) {
      return NextResponse.json(
        { error: "tier is required (starter, pro, or enterprise)", code: "VALIDATION_ERROR" },
        { status: 400 },
      );
    }
    ...
    if (!validTiers.includes(tier)) {
      return NextResponse.json(
        { error: `Invalid tier "${tier}". Must be one of: starter, pro, enterprise`, code: "VALIDATION_ERROR" },
        { status: 400 },
      );
    }
