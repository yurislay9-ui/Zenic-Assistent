...
const EXTERNAL_SERVICES = [
  {
    id: "jira",
    nombre: "Jira",
    descripcion: "REST API v2 — Crear issues, transiciones, búsqueda JQL",
    campos: ["JIRA_BASE_URL", "JIRA_EMAIL", "JIRA_API_TOKEN"],
    authType: "basic_auth",
    icon: "📋",
  },
  {
    id: "servicenow",
    nombre: "ServiceNow",
    descripcion: "Table API — Incidentes, cambios, solicitudes",
    campos: ["SERVICENOW_INSTANCE_URL", "SERVICENOW_USERNAME", "SERVICENOW_PASSWORD"],
    authType: "basic_auth",
    icon: "🔧",
  },
  {
    ...
    descripcion: "Incoming Webhooks + Bot Framework",
    campos: ["TEAMS_WEBHOOK_URL", "TEAMS_BOT_TOKEN"],
    authType: "bearer",
    icon: "💬",
  },
  {
    id: "slack",
    nombre: "Slack",
    descripcion: "Web API + Socket Mode",
    campos: ["SLACK_BOT_TOKEN", "SLACK_APP_TOKEN", "SLACK_SIGNING_SECRET"],
    authType: "bearer",
    icon: "📱",
  },
  {
    ...
    descripcion: "Meta Graph API v18.0",
    campos: ["WHATSAPP_ACCESS_TOKEN", "WHATSAPP_PHONE_NUMBER_ID", "WHATSAPP_VERIFY_TOKEN"],
    authType: "bearer",
    icon: "📲",
  },
  {
    id: "twilio",
    nombre: "Twilio SMS",
    descripcion: "REST API — Envío de SMS",
    campos: ["TWILIO_ACCOUNT_SID", "TWILIO_AUTH_TOKEN", "TWILIO_PHONE_NUMBER"],
    authType: "api_key",
    icon: "📞",
  },
  {
    id: "email",
    nombre: "Email (SMTP/Graph)",
    descripcion: "SMTP o Microsoft Graph API",
    campos: ["SMTP_HOST", "SMTP_PORT", "SMTP_USER", "SMTP_PASSWORD", "MSGRAPH_CLIENT_ID", "MSGRAPH_CLIENT_SECRET"],
    authType: "basic_auth",
    icon: "📧",
  },
  {
    ...
    descripcion: "Conexión genérica HTTP/HTTPS",
    campos: ["ENDPOINT_URL", "AUTH_CREDENTIAL"],
    authType: "custom",
    icon: "🔌",
  },
];
...
function getStatusBadge(status: string) {
  switch (status) {
    case "valid":
      return (
        <Badge className="text-[8px] px-1.5 py-0 border-0 font-bold shrink-0 bg-emerald-100 text-emerald-700">
          VÁLIDA
        </Badge>
      );
    case "invalid":
      return (
        <Badge className="text-[8px] px-1.5 py-0 border-0 font-bold shrink-0 bg-red-100 text-red-700">
          INVÁLIDA
        </Badge>
      );
    case "error":
      return (
        <Badge className="text-[8px] px-1.5 py-0 border-0 font-bold shrink-0 bg-amber-100 text-amber-700">
          ERROR
        </Badge>
      );
    default:
      return (
        <Badge className="text-[8px] px-1.5 py-0 border-0 font-bold shrink-0 bg-gray-100 text-gray-500">
          SIN PROBAR
        </Badge>
      );
  }
...
function getApiGroupColor(color: string): { bg: string; text: string; border: string; iconBg: string } {
...
// ═══════════════════════════════════════════════════════════════════════════════
// COMPONENTE: Tarjeta de Grupo API
// ═══════════════════════════════════════════════════════════════════════════════

function ApiGroupCard({ group }: { group: typeof ZENIC_API_GROUPS[0] }) {
      ...
      <CardContent className="px-4 pb-3 space-y-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {group.auth.map((a) => (
            <Badge
              key={a}
              className="text-[7px] px-1.5 py-0 border font-semibold shrink-0"
              variant="outline"
            >
        ...
        <button
          ...
          className="flex items-center gap-1 text-[9px] text-gray-400 hover:text-gray-600 transition-colors w-full"
        >
          {expandido ? (
          ...
          <div className="space-y-1 pt-1 border-t border-gray-100">
            {group.rutas.map((ruta, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex items-start gap-2 py-1 text-[9px]"
                >
                  <span className={`font-bold shrink-0 w-10 ${methodColor}`}>
                    {method}
                  </span>
                  <div className="min-w-0">
                    <span className="font-mono text-gray-600">{path[0]}</span>
                    {path[1] && (
                      <span className="text-gray-400 ml-1">— {path[1]}</span>
                    )}
                  </div>
                </div>
              );
...
// ═══════════════════════════════════════════════════════════════════════════════
// COMPONENTE: Credencial de Servicio Externo
// ═══════════════════════════════════════════════════════════════════════════════
