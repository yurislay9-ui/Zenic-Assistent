// ─── Zenic-Agents v3 — Approval Engine Query Methods ──────────────────
// Split from approval-engine.ts — standalone query/stats functions that use db directly
// These mirror the class methods but as standalone functions for modularity

import { db } from "@/lib/db";
import type { ApprovalRequest, ApprovalListOptions, ApprovalStats } from "../types";
import { ApprovalRequestStatus, ApprovalPriority, ApprovalType, DecisionType, HitlEventType } from "../types";
import { recordAuditEvent } from "../approval-audit";
import { notifyApprovalEvent } from "../notifications";
import { mapRecordToModel } from "./_engine-core";
...
export async function checkExpiredRequests(): Promise<number> {
  ...
  await Promise.all(expired.map((record) =>
    Promise.all([
      recordAuditEvent({
        requestId: record.requestId, eventType: HitlEventType.EXPIRED,
...
/** Get approval statistics — uses COUNT queries instead of findMany */
export async function getStats(): Promise<ApprovalStats> {
  const [total, byStatusRows, byPriorityRows, byTypeRows, autoApprovedCount, avgDecisionRows] = await Promise.all([
    ...
    db.hitlApprovalDecision.count({ where: { decisionBy: "system", decision: DecisionType.APPROVED } }),
...
/** List pending approvals for a specific user — filters in SQL, not JS */
export async function listPendingForUser(userId: string): Promise<ApprovalRequest[]> {
  const [decidedIds, delegatedIds] = await Promise.all([
    ...
    db.hitlDelegation.findMany({ where: { toUserId: userId, isActive: true }, select: { requestId: true } }),
  ]);
  ...
  return pendingRequests.map((r) => mapRecordToModel(r));
}

/** Get approval history with filters */
export async function getHistory(options: ApprovalListOptions & { userId?: string }): Promise<{
  data: ApprovalRequest[]; total: number; page: number; pageSize: number;
}> {
  const { status, priority, requesterId, type, targetResource, userId,
