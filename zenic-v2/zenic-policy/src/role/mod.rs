...
// identical to the old single-file module (e.g. `crate::role::Role`).
pub use manager::RoleRegistry;
pub use permissions::Role;
pub use types::{CriticalityClearance, RoleAssignment, RoleId, RolePriority};
...
mod tests {
    ...
    fn criticality_clearance_allows() {
        assert!(CriticalityClearance::Critical.allows(zenic_proto::NodeCriticality::Critical));
        assert!(CriticalityClearance::Critical.allows(zenic_proto::NodeCriticality::Low));
        assert!(CriticalityClearance::High.allows(zenic_proto::NodeCriticality::High));
        assert!(!CriticalityClearance::High.allows(zenic_proto::NodeCriticality::Critical));
        assert!(CriticalityClearance::Medium.allows(zenic_proto::NodeCriticality::Medium));
        assert!(!CriticalityClearance::Medium.allows(zenic_proto::NodeCriticality::High));
        assert!(CriticalityClearance::Low.allows(zenic_proto::NodeCriticality::Low));
        assert!(!CriticalityClearance::Low.allows(zenic_proto::NodeCriticality::Medium));
    }

    #[test]
    fn criticality_clearance_as_criticality() {
        assert_eq!(CriticalityClearance::Critical.as_criticality(), zenic_proto::NodeCriticality::Critical);
        assert_eq!(CriticalityClearance::High.as_criticality(), zenic_proto::NodeCriticality::High);
    }
    ...
    fn role_allows_criticality() {
        ...
        assert!(role.allows_criticality(zenic_proto::NodeCriticality::High));
        assert!(!role.allows_criticality(zenic_proto::NodeCriticality::Critical));
    }
    ...
    fn role_assignment_new() {
        let assignment = RoleAssignment::new(RoleId::new(), zenic_proto::SessionId::new(), zenic_proto::TenantId::new());
    ...
    fn role_registry_roles_for_session_different_tenant() {
        ...
        let sid = zenic_proto::SessionId::new();
        let tid1 = zenic_proto::TenantId::new();
        let tid2 = zenic_proto::TenantId::new();
