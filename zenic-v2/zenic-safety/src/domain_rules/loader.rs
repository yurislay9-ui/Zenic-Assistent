//! Domain rule loader — builds all 35 domain-specific safety rules.
//!
...
pub fn build_all_rules() -> Vec<DomainRule> {
    let mut rules = Vec::with_capacity(35);
    rules.extend(ai_data_rules());
    rules.extend(fintech_rules());
    rules.extend(healthtech_rules());
    rules.extend(greentech_rules());
    rules.extend(edtech_rules());
    rules.extend(proptech_rules());
    rules.extend(legaltech_rules());
    rules
}
